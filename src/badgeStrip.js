...
function buildHeaderSvg(width) {
  return Buffer.from(
    `<svg width="${width}" height="${HEADER_HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      <text x="0" y="14" font-size="14" font-weight="700" fill="#ffffff" font-family="Arial, sans-serif">Badges</text>
    </svg>`
  );
}
...
export async function renderBadgeStrip(badges) {
  ...
  const height = HEADER_HEIGHT + PADDING_Y * 2 + rows * ICON_SIZE + (rows - 1) * GAP;

  const composites = [{ input: buildHeaderSvg(width), left: PADDING_X, top: 0 }];
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    const top = HEADER_HEIGHT + PADDING_Y + row * (ICON_SIZE + GAP);
